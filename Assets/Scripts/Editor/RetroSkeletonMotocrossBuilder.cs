...
public static class RetroSkeletonMotocrossBuilder
{
    ...
    private static void ConfigureLitRenderer(DirectionalSpriteBillboardLitRenderer renderer, DirectionalSpriteAnimator animator, Renderer quadRenderer)
    {
        ...
        SetFloat(serialized, "normalScale", 1.45f);
        SetFloat(serialized, "detailNormalInfluence", 0.92f);
        SetFloat(serialized, "macroNormalBend", 1.2f);
        SetFloat(serialized, "spriteAngleLightingInfluence", 0.62f);
        SetFloat(serialized, "wrapDiffuse", 0.25f);
        SetFloat(serialized, "ambientIntensity", 0.86f);
        SetFloat(serialized, "renderSettingsAmbientScale", 0.2f);
        SetFloat(serialized, "surfaceRoughness", 0.76f);
        SetFloat(serialized, "specularStrength", 0.42f);
        SetFloat(serialized, "rimStrength", 0.42f);
        SetFloat(serialized, "rimPower", 2.7f);
        SetColor(serialized, "rimColor", new Color(1f, 0.28f, 0.08f, 1f));
        serialized.ApplyModifiedPropertiesWithoutUndo();
        ...
        Transform quad)
    {
        ...
        SetColor(serialized, "speedRimColor", new Color(1f, 0.12f, 0.02f, 1f));
        SetColor(serialized, "wheelieRimColor", new Color(1f, 0.76f, 0.18f, 1f));
